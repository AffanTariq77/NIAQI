...
const ProfileScreen = () => {
  ...
  // Refresh user data when profile screen mounts
  useEffect(() => {
    // Only refresh if user is authenticated
    if (user && user.id) {
      console.log("👤 Profile screen: Refreshing user data...");
      refreshUser();
    } else {
      console.log("👤 Profile screen: User not authenticated yet");
    }
  }, [user?.id]);
  ...
  const handleMembershipPress = () => {
    // Navigate to membership details with current membership type
    router.push({
  ...
  const handleUpgradePlan = () => {
    // Navigate to upgrade screen or home to see packages
    router.push("/upgrade");
  };
  ...
  const getMembershipDisplay = () => {
    switch (user?.membershipType) {
      ...
      case "PREMIUM":
        return { name: "Premium", color: "#5A7CFF", icon: "star" as const };
      case "BASIC":
        return {
          name: "Basic",
          color: "#10B981",
            ...
            <View style={styles.userCardContent}>
              ...
              <View style={styles.userInfo}>
                ...
                <Text style={styles.userEmail}>
                  {user?.email || "heather.ues@gmail.com"}
                </Text>
              </View>
            </View>
